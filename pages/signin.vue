  ...
  <v-container grid-list-md>
    <v-layout
      ...
      <v-flex xs12 sm7 md6 lg4 xl3>
        <v-card>
          <v-progress-linear
            v-if="this.$store.state.checkuser.isFindPending"
          ...
          <v-card-title primary-title class="card_title_login">
            <div style="width: 100%">
              ...
              <div style="text-align: center;">
                ...
                <div v-if="$store.getters['checkuser/list'].length === 0">Aplikasi Terintegrasi Kabupaten Purbalingga</div>
              </div>
            </div>
          </v-card-title>
          <v-card-text class="card_text_login">
            <v-flex  v-if="$store.getters['checkuser/list'].length === 0">
              <v-text-field
            ...
            <div v-if="$store.getters['checkuser/list'].length > 0">
              <h3>Selamat Datang,</h3>
              <div>{{ $store.getters['checkuser/list'][0].fullname }}</div>
              <br />
              <v-flex>
                <v-text-field
          ...
          <v-card-actions class="card_action_login">
            <v-spacer></v-spacer>
            <v-btn
              v-if="$store.getters['checkuser/list'].length === 0"
              color="primary"
    ...
    computed: {
      errorMessageUsername: function () {
        if (this.$store.state.checkuser.errorOnFind !== null) {
          return this.$store.state.checkuser.errorOnFind.message
        } else {
          return this.errors.collect('username')
        }
      },
      errorMessagePassword: function () {
        if (this.$store.state.auth.errorOnAuthenticate !== null) {
